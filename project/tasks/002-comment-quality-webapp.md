...
- 修复运行中任务列表进度不更新的问题：每条评论完成或失败后立即重算并持久化 `successRows/failedRows`。
- 读取任务时会按明细行状态校正摘要计数，避免旧数据中明细已完成但列表仍显示 `0/总数`。
- 前端在存在运行中任务时每 2 秒刷新任务摘要和当前任务详情。
- `npm test`：通过，覆盖运行中单条完成后任务列表摘要从 `0/2` 更新为 `1/2`，以及旧摘要计数按行状态校正。
- `npm run typecheck`：通过。
- `npm run build`：通过。
